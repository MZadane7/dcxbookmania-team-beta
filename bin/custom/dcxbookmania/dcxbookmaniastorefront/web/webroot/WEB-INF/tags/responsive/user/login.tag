...
<form:form action="${action}" method="post" modelAttribute="loginForm">
	...
	<formElement:formPasswordBox idKey="j_password"
		...
		mandatory="true" />
<label for="groups">Choose your Group:</label>

	<select id="groups" name="groups">

		<option value="premiumgroup">Premium</option>

		<option value="progroup">Pro</option>

		<option value="primegroup">Prime</option>

		<option value="plusgroup">Plus</option>

		<option value="regulargroup">Regular</option>
	...
	<c:if test="${expressCheckoutAllowed}">
		<button type="submit"
			class="btn btn-default btn-block expressCheckoutButton">
			<spring:theme code="text.expresscheckout.header" />
		</button>
		<input id="expressCheckoutCheckbox" name="expressCheckoutEnabled"
			type="checkbox" class="form left doExpressCheckout display-none" />
	</c:if>
